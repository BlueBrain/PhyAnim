...
OverlapScene::OverlapScene(const std::vector<std::string>& files_,
    ...
    , _camera(camera_)
    , _dt(dt_)
    , _meshStiffness(meshStiffness_)
    , _meshDensity(meshDensity_)
    , _meshDamping(meshDamping_)
...
unsigned int OverlapScene::_compileShader(const std::string& source_, int type_)
{
    unsigned int shader;
