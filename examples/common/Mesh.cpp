...
void Mesh::uploadPositions(std::vector<double> positions)
{
    _positions = positions;
    _updatePositions = true;
}
void Mesh::uploadNormals(std::vector<double> normals)
{
    _normals = normals;
    _updateNormals = true;
}
void Mesh::uploadColors(std::vector<double> colors)
{
    _colors = colors;
    _updateColors = true;
}
...
{
    glBindBuffer(GL_ARRAY_BUFFER, _positionVbo);
    glBufferData(GL_ARRAY_BUFFER, sizeof(double) * _positions.size(),
...
{
    glBindBuffer(GL_ARRAY_BUFFER, _normalVbo);
    glBufferData(GL_ARRAY_BUFFER, sizeof(double) * _normals.size(),
...
{
    glBindBuffer(GL_ARRAY_BUFFER, _colorVbo);
    glBufferData(GL_ARRAY_BUFFER, sizeof(double) * _colors.size(),
...
void setGeometry(Mesh* mesh, phyanim::Nodes& nodes)
{
    ...
    std::vector<double> positions(numNodes * 3);
    std::vector<double> normals(numNodes * 3);
...
void setColor(Mesh* mesh, uint32_t numNodes, phyanim::Vec3 color)
{
    std::vector<double> colors(numNodes * 3);
    for (uint32_t i = 0; i < numNodes; ++i)
    {
...
void setColorByCollision(Mesh* mesh,
                         phyanim::Nodes& nodes,
                         phyanim::Vec3 noCollisionColor,
                         phyanim::Vec3 collisionColor)
{
    uint32_t numNodes = nodes.size();
    std::vector<double> colors(numNodes * 3);
    for (uint32_t i = 0; i < numNodes; ++i)
                   ...
                   phyanim::Vec3 fixColor)
{
    ...
    std::vector<double> sectionNormals;
    std::vector<double> sectionColors;
    for (auto edge : edges)
    {
        computeEdgeIndices(sectionIndices, positions.size() / 3);
        computeEdgeGeometry(dynamic_cast<phyanim::Edge*>(edge),
                         ...
                         phyanim::Vec3 fixColor)
{
    ...
    auto t = (p1 - p0).normalized();

    if (t.norm() <= 0.0) t = phyanim::Vec3(0, 1, 0);
    phyanim::Vec3 axis_y(0, 1, 0);
    if (t.y() == 1.0 || t.y() == -1.0) axis_y = phyanim::Vec3(-1, 0, 0);

    phyanim::Vec3 axis_x = axis_y.cross(t).normalized();
    axis_y = t.cross(axis_x).normalized();
