...
{
    bool rightCopy =
        (mesh0_->nodes.size() == mesh1_->nodes.size()) &&
        (mesh0_->triangles.size() == mesh1_->triangles.size()) &&
        (mesh0_->surfaceTriangles.size() == mesh1_->surfaceTriangles.size()) &&
    ...
    if (rightCopy)
    {
        ...
        for (unsigned int i = 0; i < mesh0_->tetrahedra.size(); i++)
        {
            ...
            if (*tet0->node0 != *tet1->node0 || *tet0->node1 != *tet1->node1 ||
                *tet0->node2 != *tet1->node2 || *tet0->node3 != *tet1->node3)
            {
                rightCopy = false;
                break;
            }
