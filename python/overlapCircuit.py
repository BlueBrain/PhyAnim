...
class Overlap(App):
    ...
    def add_models(self, index_file, sonata_file, population_name, lines=False):
        ...
        self.aabb.add_pos(self.min)
        self.aabb.add_pos(self.max)
        gids = get_spatial_indices(index_file, self.min, self.max)
        print(LINE_CLEAR, end='')
        print("\rLoading {:d} neurons".format(len(gids)))
        self.num_models = float(len(gids))
        morpho_models = get_morpho_model(sonata_file, population_name, gids)

        prev_time = time.time()

        self.total_lenght = 0.0
        for i, morpho_model in enumerate(morpho_models):
            morphology = get_morpho(morpho_model[0])
            model = morpho_model[1]
            color = get_color(i)
            morpho = Morphology(morphology, model, color)
            # morpho.cut(self.aabb)
            nodes, springs = morpho.get_springs(self.ks, self.kd, self.aabb) 
        ...
        for i,set in enumerate(self.spring_sets):
            color = get_color(i)
            mesh = mesh_springs(set.springs, color*0.5, color)
            self.meshes.append(mesh)
            self.scene.add_model((mesh, self.program, mat4()))

        self.scene.update_models()
        self.scene.target = self.aabb.center()
        self.scene.radius = self.aabb.radius() * 1.5
        ...
        print("Loaded " + str(self.loaded_models) + "/" + str(len(gids)) +
                ...
                if self.solving:
                    ...
                    if n:
                        ...
                        if self.verbose:
                            ...
                            print("\r", end="")
                        print("- Collisions {:d}/{:d} solved in {:.04f} sc. Length {:.2f}/{:.2f} - diff: {:.4f}%".format(
                    ...
                    if self.col < len(self.collisions):
                        ...
                        if self.verbose:
                            print(end=LINE_CLEAR)
                            print("\r")
                        print("-- {:d} collisions solved in {:.04f} sc. Length {:.2f}/{:.2f} - diff: {:.4f}%".format(
                            len(self.collisions),
...
if __name__ == "__main__":
    ...
    app.add_models(
        args.index_file, args.sonata_file, args.population_name)
    app.scene.level = 5
