...
AABBNode::AABBNode(AABBNode* other_) 
    : aabb(other_->aabb), child0(nullptr), child1(nullptr) {
}
...
void AABBNode::outterNodes(Nodes& nodes_,
                           const AABB& aabb_) {
...
Vec3 AABBNode::_center(Triangle* triangle_) {
    Vec3 center = (triangle_->node0->position + triangle_->node1->position +
                   triangle_->node2->position) *0.333;
    return center;
}
...
AxisAlignedBoundingBox::AxisAlignedBoundingBox(
    Nodes& nodes_, Triangles& triangles_, Tetrahedra& tetrahedra_) {
    root = new AABBNode();
...
void AxisAlignedBoundingBox::generate(Nodes& nodes_, Triangles& triangles_,
