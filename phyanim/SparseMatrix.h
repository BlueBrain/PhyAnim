 ...
 * Sparse Matrix
 */
template<uint32_t M, uint32_t N, typename T, typename =
...
public:
    ...
    Vec<N, T> operator *(const Vec<N, T>& v) const
    {
        ...
        for(uint32_t i = 0; i < M; ++i)
            for (auto ele: rows[i])
                w[i] += ele.second * v[ele.first.second];
        return w;
    }
 ...
 */
template<uint32_t N, typename T, typename =
         typename std::enable_if<std::is_arithmetic<T>::value, T>::type>
...
public:
    ...
    /**
     * @brief
     * Solve the A * x = b linear equation system through a Conjungate Gradient approach
     * @param A
     * Matrix of MxN size 
     * @param b
     * Vector of M size 
     ...
     * Vector of N size
     */
    Vec<N, T> solve(SparseMatrix<N, N, T> A, Vec<N, T> b, T threshold = 1e-10,
