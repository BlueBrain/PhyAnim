...
const std::string fSkySource(
    ...
    "float inc = 0.005;"
    "vec3 dir = pos;"
    "vec4 difuse = texture(sky, dir) * 0.5;"
    // "int size = 5;"
    // "for(int i = 0; i < size; ++i)"
    // "for(int j = 0; j < size; ++j){"
    // "vec3 incVec = vec3(inc*(i-size*0.5), inc*(j - size*0.5), 0.0);"
    // "difuse += texture(sky, dir + incVec);}"
    // "difuse *= 0.2/size;"
    "color = difuse;}");
...
SkyBox::SkyBox(const std::string& file)
{
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * numPos, pos, GL_STATIC_DRAW);
...
void SkyBox::render(Camera* camera)
{
    ...
    glUniformMatrix4fv(_uView, 1, GL_FALSE,
                       glm::value_ptr(camera->viewMatrix()));
    glUniformMatrix4fv(_uProj, 1, GL_FALSE,
